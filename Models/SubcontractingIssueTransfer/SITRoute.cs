...
{
    // RII_SIT_ROUTE tablosu:
    // Üretim veya transfer sürecinde, her satırın (SitLine) izlediği rotayı (kaynak → hedef) tanımlar.
    // Rota, malzemenin hangi depodan/hücreden nereye taşındığını, hangi sırayla işlendiğini belirtir.
    [Table("RII_SIT_ROUTE")]
    public class SitRoute : BaseEntity
    {
        // Bu rota hangi SitLine kaydına bağlı (örneğin üretim satırı veya transfer kalemi)
        [Required, ForeignKey(nameof(Line))]
        public long LineId { get; set; }

        // Navigation property – rota, bağlı olduğu satıra (SitLine) erişim sağlar
